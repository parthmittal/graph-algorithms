...
struct sk_hack_t {
    ...
    sk_hack_t(const our::graph_t<int> &G, const std::vector<int> &P);
    void add_sp_edges(int source);
    void add_vertex(int v);
};

struct sk_graph_t : sk_hack_t, graph<wm_edge_t> {
    sk_graph_t(const graph_t<int> &G, const std::vector<int> &P)
        : sk_hack_t(G, P), graph<wm_edge_t>(adj) {}
};
